...
func TestTiCIBuildPathUsesStructuredBooleanQuery(t *testing.T) {
	...
	ds, indexInfo := newTestTiCIDataSource(t, ctx, buildTestMatchAgainst(t, ctx, `+a -b c baz*`))
	...
	require.Len(t, ftsQueryInfo.BooleanQuery.Nodes, 4)
	require.Len(t, ftsQueryInfo.MatchExpr, 0)
...
func TestTiCIBuildPathFallsBackForNullPattern(t *testing.T) {
	ctx := mockctx.NewContext()
	ctx.Store = &mockctx.Store{Client: &mockctx.Client{}}

	ds, indexInfo := newTestTiCIDataSource(t, ctx, buildTestMatchAgainstNullPattern(t, ctx))
	matchedExprSet := intset.NewFastIntSet()
	matchedExprSet.Insert(0)
	...
	require.NotNil(t, ftsQueryInfo.MatchExpr[0])
	require.Empty(t, findSchemaColumnByID(ds.Schema(), model.VirtualColFTSScoreID))
}
...
func buildTestMatchAgainstNullPattern(t *testing.T, ctx *mockctx.Context) *expression.ScalarFunction {
	t.Helper()
