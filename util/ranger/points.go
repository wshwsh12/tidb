...
func (r *builder) buildFormBinOp(expr *expression.ScalarFunction) []point {
	...
	if ft.Tp == mysql.TypeEnum && !mysql.HasEnumSetAsIntFlag(ft.Flag) {
		return handleEnumBinOp(ft, value, op)
	}
...
func handleEnumBinOp(ft *types.FieldType, val types.Datum, op string) []point {
